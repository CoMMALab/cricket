...
This branch supports generating, tracing constraints along with their gradients. Constraints are generated by default for robots. Feel free to comment out the code related to constraint generation in `fkcc_gen.cc`. Codegen can sometimes produce conditional branches for certain parts of the code, so `fix_robot.py` is a simple regex helper to replace if-else conditions with simd friendly blend statements. It is a bit of a hack, but will be more gracefully handled soon. Constraint are defined in `tsr_constraints.hh`
