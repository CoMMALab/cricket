...
private:
    ...
    std::size_t _input_total;

    std::vector<VarSegment> _output_segments;
    std::vector<std::size_t> _output_starts;
    std::size_t _output_total;

    mutable std::stringstream _ss;

public:
    // Constructor with only input segments (backward compatible)
    ...
    // Constructor with both input and output segments
      ...
      , _output_total(0)
    {
        ...
        // Set up function arguments for input segments
        ...
        // Set up function arguments for output segments (only if custom outputs provided)
    ...
    const std::string &getIndependentArrayName(const CppAD::cg::OperationNode<Base> &indep, size_t id) override
    {
        std::size_t index = id - 1;
        auto [seg_idx, local_idx] = findInputSegment(index);
        if (seg_idx < _input_segments.size())
    ...
    std::size_t getIndependentArrayIndex(const CppAD::cg::OperationNode<Base> &indep, std::size_t id) override
    {
        std::size_t index = id - 1;
        auto [seg_idx, local_idx] = findInputSegment(index);
        return local_idx;
        ...
        std::size_t idSecond) override
    {
        ...
        return seg1 == seg2 && i1 + 1 == i2 && _input_segments[seg1].is_array;
    }
