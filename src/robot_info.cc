...
auto RobotInfo::extract_spheres() -> void
{
    ...
    for (auto i = 0U; i < model.frames.size(); ++i)
    {
        ...
        if (not link_info.empty())
        {
            ...
            info.relative = pinocchio::SE3::Identity();
...
auto RobotInfo::collision_pair_to_frame_pair(const pinocchio::CollisionPair &cp)
...
auto RobotInfo::guess_self_collisions(std::size_t n) -> void
{
    ...
    {
        collision_model.addCollisionPair(pinocchio::CollisionPair(pair.first, pair.second));
    }
}

auto RobotInfo::add_mimic_joint(
    const std::string &name, const std::string &joint, double multiplier, double offset) -> void
{
    pinocchio::Model temp;
