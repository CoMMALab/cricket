...
AD<CppAD::cg::CG<double>> lu_vec_ad(
   size_t                           n,
   size_t                           m,
   CppAD::VecAD<CppAD::cg::CG<double>>             &Matrix,
   CppAD::VecAD<CppAD::cg::CG<double>>             &Rhs,
   CppAD::VecAD<CppAD::cg::CG<double>>             &Result,
   CppAD::AD<CppAD::cg::CG<double>>                &logdet)
{
   ...
   for(p = 0; p < N; p += 1.)
   {
      ...
      const auto imax_value = Value(imax);
      index    = ip[p];
      ip[p]    = ip[imax_value];
      ip[imax_value] = index;
      ...
      const auto jmax_value = Value(jmax);
      jndex    = jp[p];
      jp[p]    = jp[jmax_value];
      jp[jmax_value] = jndex;
