        ...
        namespace constants
        {
            namespace detail
            {
                ...
                struct constant_pi<CppAD::cg::CG<Scalar>> : constant_pi<Scalar>
                {
                    ...
                    template <int N>
                    inline static CGScalar get(const mpl::int_<N> &n)
                    {
                    ...
                    inline static CGScalar get(const std::integral_constant<T, value> &n)
                    {
                        return CGScalar(constant_pi<Scalar>::get(n));
                    }
#else
                    template <class T, T value>
                    inline static CGScalar get(const boost::integral_constant<T, value> &n)
                    {
                        return CGScalar(constant_pi<Scalar>::get(n));
                    }
#endif
...
namespace Eigen
{
    namespace internal
    {
        ...
        struct cast_impl<CppAD::cg::CG<Scalar>, Scalar>
        {
            ...
            EIGEN_DEVICE_FUNC
#endif
            inline static Scalar run(const CppAD::cg::CG<Scalar> &x)
            {
                return x.getValue();
            }
        };
        ...
        struct cast_impl<CppAD::AD<CppAD::cg::CG<Scalar>>, Scalar>
        {
            ...
            EIGEN_DEVICE_FUNC
#endif
            inline static Scalar run(const CppAD::AD<CppAD::cg::CG<Scalar>> &x)
            {
