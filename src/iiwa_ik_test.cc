...
int main()
{
    ...
    const auto result = IiwaBimanualParameterization<double, Eigen::VectorXd>(ad_inp);

    for (std::size_t i = 0; i < result.q.size(); ++i)
    {
        std::cout << "y[" << i << "] = " << result.q[i] << '\n';
    }
    for (std::size_t i = 0; i < result.unclipped.size(); ++i)
    {
        std::cout << "unclipped[" << i << "] = " << result.unclipped[i] << '\n';
    }
    ...
    const auto result_se3 = IiwaSE3Parameterization<double, Eigen::VectorXd>(ad_inp_se3);

    for (std::size_t i = 0; i < result_se3.q.size(); ++i)
    {
        std::cout << "y_se3[" << i << "] = " << result_se3.q[i] << '\n';
    }
    for (std::size_t i = 0; i < result_se3.unclipped.size(); ++i)
    {
        std::cout << "unclipped_se3[" << i << "] = " << result_se3.unclipped[i] << '\n';
    }
